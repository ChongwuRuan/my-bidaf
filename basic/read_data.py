...
def read_data(config, data_type, ref, data_filter=None):
    ...
    if not ref:
        ...
        syn_counter = shared['syn_counter']
        if config.finetune:
        ...
        shared['syn2idx'] = {syn: idx + 2 for idx, syn in
                             enumerate(syn for syn, count in syn_counter.items() if count > config.syn_count_th)}
        NULL = "-NULL-"
        UNK = "-UNK-"
        ...
        shared['syn2idx'][NULL] = 0
        shared['syn2idx'][UNK] = 1
        json.dump({'word2idx': shared['word2idx'], 'char2idx': shared['char2idx'], 'syn2idx': shared['syn2idx']},
                  open(shared_path, 'w'))
    else:
...
def update_config(config, data_sets):
    ...
    config.syn_vocab_size = len(data_sets[0].shared['syn2idx'])
