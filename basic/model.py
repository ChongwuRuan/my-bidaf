...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        self.synx = tf.placeholder('int32', [N, None, None, 10], name='synx')
        self.synx_mask = tf.placeholder('bool', [N, None, None, 10], name='synx_mask')
        self.q = tf.placeholder('int32', [N, None], name='q')
        self.cq = tf.placeholder('int32', [N, None, W], name='cq')
        self.synq = tf.placeholder('int32', [N, None, 10], name='synq')
        self.synq_mask = tf.placeholder('bool', [N, None, 10], name='synq_mask')
        ...
        self.syn_emb_mat = tf.random_normal(shape=[VS, 15], dtype='float')
    ...
    def _build_forward(self):
        ...
        syn_cell_fw = BasicLSTMCell(30, state_is_tuple=True)
        syn_cell_bw = BasicLSTMCell(30, state_is_tuple=True)
        d_syn_cell_fw = SwitchableDropoutWrapper(syn_cell_fw, self.is_train, input_keep_prob=config.input_keep_prob)
        d_syn_cell_bw = SwitchableDropoutWrapper(syn_cell_bw, self.is_train, input_keep_prob=config.input_keep_prob)
        ...
        synx_len = tf.reduce_sum(tf.cast(self.synx_mask, 'int32'), 3)
        synq_len = tf.reduce_sum(tf.cast(self.synq_mask, 'int32'), 2)

        with tf.variable_scope("syn_emb"):
            synx = tf.nn.embedding_lookup(self.syn_emb_mat, self.synx)
            synq = tf.nn.embedding_lookup(self.syn_emb_mat, self.synq)
            _, (fw_fh, bw_fh) = bidirectional_dynamic_rnn(d_syn_cell_fw, d_syn_cell_bw, synq, synq_len, dtype='float',
                                                          scope='synq')
            synq_vec = tf.concat([fw_fh, bw_fh], 2)
            if config.share_lstm_weights:
                tf.get_variable_scope().reuse_variables()
                _, (fw_fh, bw_fh) = bidirectional_dynamic_rnn(syn_cell_fw, syn_cell_bw, synx, synx_len, dtype='float',
                                                              scope='synq')
                synx_vec = tf.concat([fw_fh, bw_fh], 3)
            else:
                _, (fw_fh, bw_fh) = bidirectional_dynamic_rnn(syn_cell_fw, syn_cell_bw, synx, synx_len, dtype='float',
                                                              scope='synx')
                synx_vec = tf.concat([fw_fh, bw_fh], 3)

        xx = tf.concat([xx, synx_vec], 3)
        qq = tf.concat([qq, synq_vec], 2)

        self.tensor_dict['xx'] = xx
        self.tensor_dict['qq'] = qq

        with tf.variable_scope("prepro"):
            (fw_u, bw_u), _ = bidirectional_dynamic_rnn(d_cell_fw, d_cell_bw, qq, q_len, dtype='float',
                                                        scope='u1')  # [N, J, d], [N, d]
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        synx = np.zeros([N, M, JX, 10], dtype='int32')
        synx_mask = np.zeros([N, M, JX, 10], dtype='bool')
        q = np.zeros([N, JQ], dtype='int32')
        cq = np.zeros([N, JQ, W], dtype='int32')
        q_mask = np.zeros([N, JQ], dtype='bool')
        synq = np.zeros([N, JQ, 10], dtype='int32')
        synq_mask = np.zeros([N, JQ, 10], dtype='bool')

        feed_dict[self.x] = x
        feed_dict[self.x_mask] = x_mask
        feed_dict[self.cx] = cx
        feed_dict[self.synx] = synx
        feed_dict[self.synx_mask] = synx_mask
        feed_dict[self.q] = q
        feed_dict[self.cq] = cq
        feed_dict[self.q_mask] = q_mask
        feed_dict[self.synq] = synq
        feed_dict[self.synq_mask] = synq_mask
        ...
        for i, synxi in enumerate(batch.data['syn_seq']):
            for j, synxij in enumerate(synxi):
                if j == config.max_num_sents:
                    break
                for k, synxijk in enumerate(synxij):
                    if k == config.max_sent_size:
                        break
                    for l, synxijkl in enumerate(synxijk):
                        synx[i, j, k, l] = _get_syn(synxijkl)
                        if synxijkl is not 'ROOT':
                            synx_mask[i, j, k, l] = True

        for i, synqi in enumerate(batch.data['q_syn_seq']):
            for j, synqij in enumerate(synqi):
                for k, synqijk in enumerate(synqij):
                    synq[i, j, k] = _get_syn(synqijk)
                    if synqijk is not 'ROOT':
                        synq_mask[i, j, k] = True
